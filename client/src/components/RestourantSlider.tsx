...
const CardSlider: React.FC = () => {
  ...
  useEffect(() => {
    const fetchRestaurants = async () => {
      try {
        const response = await axios.get('http://localhost:2560/api/v1/homepage/nofooditems');
    ...
    <div className="max-w-7xl mx-auto p-2">
      <div className="relative">
        <div className="flex items-center">
          <h2 className="text-black font-semibold mr-auto">Top Restaurants Near SRM</h2>
          <button className="bg-gray-100 rounded-full text-black p-2 ml-2" onClick={scrollLeft}>
            <Image src="/assets/arrow-left.svg" alt="Scroll Left" width={16} height={16} />
          </button>
          <button className="bg-gray-100 rounded-full text-black p-2 ml-2" onClick={scrollRight}>
            <Image src="/assets/arrow-right.svg" alt="Scroll Right" width={16} height={16} />
          </button>
        </div>
        <div className="flex gap-8 mt-4 overflow-x-auto no-scrollbar scroll-snap-x" ref={containerRef}>
          {restaurants.map((restaurant) => (
            <div key={restaurant._id} className="transform transition-transform hover:scale-95 min-w-[350px]">
              <div className="relative">
                <img
              ...
              <div className="flex justify-between text-base">
                ...
                <div className="flex items-center">
                  <i className="bi bi-dot"></i> {restaurant.deliveryTime || '20-30 mins'}
                </div>
              </div>
